...
//  Fresh
//
//	Adapted from http://code.google.com/p/polypartition/
//
//  Created by Jeff Wofford on 6/12/13.
//  Copyright (c) 2013 Jeff Wofford. All rights reserved.
//

#ifndef Fresh_PolygonPartition_h
#define Fresh_PolygonPartition_h
	...
	namespace partition
	{
		...
		public:
			...
			Poly(const Poly &src) {
				...
				points = new point_t[numpoints];
				memcpy(points, src.points, numpoints*sizeof(point_t));
			}
			...
			static const int _CCW = 1;
			static const int _CW = -1;
			...
			int GetOrientation() {
				...
				if(area>0) return _CCW;
				if(area<0) return _CW;
				return 0;
			}
			...
			void Invert() {
				...
				invpoints = new point_t[numpoints];
		...
		protected:
			...
			typedef PolygonT polygon_t;
			typedef typename PolygonT::point_t point_t;
			typedef typename PolygonT::real_t real_t;
			
			typedef std::vector< polygon_t > PolyList;
			...
			struct ScanLineEdge {
				...
				bool IsConvex(const point_t& p1, const point_t& p2, const point_t& p3) const {
					real_t tmp;
			...
			point_t Normalize(const point_t &p) {
				point_t r;
				real_t n = std::sqrt(p.x*p.x + p.y*p.y);
				if(n!=0) {
					r = p/n;
				} else {
					r.x = 0;
			...
			real_t Distance(const point_t &p1, const point_t &p2) {
				real_t dx,dy;
				dx = p2.x - p1.x;
				dy = p2.y - p1.y;
			...
			bool IsConvex(point_t& p1, point_t& p2, point_t& p3) {
				real_t tmp;
				tmp = (p3.y-p1.y)*(p2.x-p1.x)-(p3.x-p1.x)*(p2.y-p1.y);
				if(tmp>0) return 1;
				else return 0;
			...
			bool IsReflex(point_t& p1, point_t& p2, point_t& p3) {
				real_t tmp;
				tmp = (p3.y-p1.y)*(p2.x-p1.x)-(p3.x-p1.x)*(p2.y-p1.y);
				if(tmp<0) return 1;
				else return 0;
			...
			bool IsInside(point_t& p1, point_t& p2, point_t& p3, point_t &p) {
			...
			bool InCone(point_t &p1, point_t &p2, point_t &p3, point_t &p) {
				bool convex;
			...
			bool InCone(PartitionVertex *v, point_t &p) {
				point_t p1,p2,p3;
			...
			void UpdateVertex(PartitionVertex *v, PartitionVertex *vertices, size_t numvertices) {
				size_t i;
				PartitionVertex *v1,*v3;
				point_t vec1,vec3;
		...
		public:
			...
			//simple heuristic procedure for removing holes from a vector of polygons
			//works by creating a diagonal from the rightmost hole vertex to some visible vertex
			//time complexity: O(h*(n^2)), h is the number of holes, n is the number of vertices
			//space complexity: O(n)
			//params:
			//   inpolys : a vector of polygons that can contain holes
			//             vertices of all non-hole polys have to be in counter-clockwise order
			//             vertices of all hole polys have to be in clockwise order
			//   outpolys : a vector of polygons without holes
			//returns 1 on success, 0 on failure
			int RemoveHoles(PolyList *inpolys, PolyList *outpolys) {
				PolyList polys;
				typename PolyList::iterator holeiter,polyiter,iter,iter2;
				size_t i,i2,holepointindex,polypointindex;
				point_t holepoint,polypoint,bestpolypoint;
				point_t linep1,linep2;
				point_t v1,v2;
				polygon_t newpoly;
				bool hasholes;
				bool pointvisible;
			...
			int Triangulate_EC(polygon_t *poly, PolyList *triangles) {
				size_t numvertices;
			...
			int Triangulate_EC(PolyList *inpolys, PolyList *triangles) {
				PolyList outpolys;
				typename PolyList::iterator iter;
			...
			int ConvexPartition_HM(polygon_t *poly, PolyList *parts) {
				PolyList triangles;
				typename PolyList::iterator iter1,iter2;
				polygon_t *poly1,*poly2;
				polygon_t newpoly;
				point_t d1,d2,p1,p2,p3;
				size_t i11,i12,i21,i22,i13,i23,j,k;
				bool isdiagonal;
				size_t numreflex;
			...
			int ConvexPartition_HM(PolyList *inpolys, PolyList *parts) {
				PolyList outpolys;
				typename PolyList::iterator iter;
			...
			int Triangulate_OPT(polygon_t *poly, PolyList *triangles) {
				size_t i,j,k,gap,n;
				DPState **dpstates;
				point_t p1,p2,p3,p4;
				size_t bestvertex;
			...
			void UpdateState(size_t a, size_t b, size_t w, size_t i, size_t j, DPState2 **dpstates) {
			...
			void TypeA(size_t i, size_t j, size_t k, PartitionVertex *vertices, DPState2 **dpstates) {
				std::vector<Diagonal> *pairs;
				typename std::vector<Diagonal>::iterator iter,lastiter;
				size_t top;
				size_t w;
			...
			void TypeB(size_t i, size_t j, size_t k, PartitionVertex *vertices, DPState2 **dpstates) {
				std::vector<Diagonal> *pairs;
				typename std::vector<Diagonal>::iterator iter,lastiter;
				size_t top;
				size_t w;
			...
			int ConvexPartition_OPT(polygon_t *poly, PolyList *parts) {
				point_t p1,p2,p3,p4;
				PartitionVertex *vertices;
				DPState2 **dpstates;
				size_t i,j,k,n,gap;
				std::vector<Diagonal> diagonals,diagonals2;
				Diagonal diagonal,newdiagonal;
				std::vector<Diagonal> *pairs,*pairs2;
				typename std::vector<Diagonal>::iterator iter,iter2;
				int ret;
				polygon_t newpoly;
				std::vector<size_t> indices;
				std::vector<size_t>::iterator iiter;
				bool ijreal,jkreal;
				...
				while(!diagonals.empty()) {
					...
					newpoly.Init((size_t)indices.size());
					k=0;
			...
			int MonotonePartition(PolyList *inpolys, PolyList *monotonePolys)
			{
				typename PolyList::iterator iter;
				MonotoneVertex *vertices;
				size_t i,numvertices,vindex,vindex2,newnumvertices,maxnumvertices;
				size_t polystartindex, polyendindex;
				polygon_t *poly;
				...
				size_t *priority = new size_t [numvertices];
				...
				for(i=0;i<numvertices;i++) {
					...
					if(Below(vprev->p,v->p)&&Below(vnext->p,v->p)) {
						if(IsConvex(vnext->p,vprev->p,v->p)) {
							vertextypes[i] = VertexType::Start;
						} else {
							vertextypes[i] = VertexType::Split;
						}
					} else if(Below(v->p,vprev->p)&&Below(v->p,vnext->p)) {
						if(IsConvex(vnext->p,vprev->p,v->p))
						{
							vertextypes[i] = VertexType::End;
						} else {
							vertextypes[i] = VertexType::Merge;
						}
					} else {
						vertextypes[i] = VertexType::Regular;
					}
				}
				
				//helpers
				size_t *helpers = new size_t[maxnumvertices];
				...
				typename std::set<ScanLineEdge>::iterator *edgeTreeIterators,edgeIter;
				edgeTreeIterators = new typename std::set<ScanLineEdge>::iterator[maxnumvertices];
				std::pair< typename std::set<ScanLineEdge>::iterator,bool> edgeTreeRet;
				...
				for(i=0;i<numvertices;i++) {
					...
					switch(vertextypes[vindex]) {
						...
						case VertexType::End:
							...
							if(vertextypes[helpers[v->previous]]==VertexType::Merge) {
						...
						case VertexType::Merge:
							...
							if(vertextypes[helpers[edgeIter->index]]==VertexType::Merge) {
			...
			//adds a diagonal to the doubly-connected vector of vertices
			void AddDiagonal(MonotoneVertex *vertices, size_t *numvertices, size_t index1, size_t index2) {
				size_t newindex1,newindex2;
			...
			bool Below(point_t &p1, point_t &p2) {
			...
			int TriangulateMonotone(polygon_t *inPoly, PolyList *triangles) {
				size_t i,i2,j,topindex,bottomindex,leftindex,rightindex,vindex;
				point_t *points;
				size_t numpoints;
				polygon_t triangle;
				...
				size_t *priority = new size_t[numpoints];
				
				//merge left and right vertex chains
				priority[0] = topindex;
				vertextypes[topindex] = 0;
				leftindex = topindex+1; if(leftindex>=numpoints) leftindex = 0;
				rightindex = topindex-1; if(rightindex>=numpoints) rightindex = numpoints-1;
				for(i=1;i<(numpoints-1);i++) {
					if(leftindex==bottomindex) {
						priority[i] = rightindex;
						rightindex--; if(rightindex>=numpoints) rightindex = numpoints-1;
				...
				size_t *stack = new size_t[numpoints];
				size_t stackptr = 0;
			...
			int Triangulate_MONO(PolyList *inpolys, PolyList *triangles) {
				PolyList monotone;
				typename PolyList::iterator iter;
			...
			int Triangulate_MONO(polygon_t *poly, PolyList *triangles) {
				PolyList polys;
