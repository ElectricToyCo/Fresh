...
ALvoid aluMixData(ALCdevice *device, ALvoid *buffer, ALsizei size)
{
    ...
    while(size > 0)
    {
        ...
        if(buffer)
        {
            ...
            {
                case DevFmtByte:
                    Write_ALbyte((const ALfloatBUFFERSIZE*) device->DryBuffer, buffer, SamplesToDo, device->NumChannels);
                    buffer = (char*)buffer + SamplesToDo*device->NumChannels*sizeof(ALbyte);
                    break;
                case DevFmtUByte:
                    Write_ALubyte((const ALfloatBUFFERSIZE*) device->DryBuffer, buffer, SamplesToDo, device->NumChannels);
                    buffer = (char*)buffer + SamplesToDo*device->NumChannels*sizeof(ALubyte);
                    break;
                case DevFmtShort:
                    Write_ALshort((const ALfloatBUFFERSIZE*) device->DryBuffer, buffer, SamplesToDo, device->NumChannels);
                    buffer = (char*)buffer + SamplesToDo*device->NumChannels*sizeof(ALshort);
                    break;
                case DevFmtUShort:
                    Write_ALushort((const ALfloatBUFFERSIZE*) device->DryBuffer, buffer, SamplesToDo, device->NumChannels);
                    buffer = (char*)buffer + SamplesToDo*device->NumChannels*sizeof(ALushort);
                    break;
                case DevFmtInt:
                    Write_ALint((const ALfloatBUFFERSIZE*) device->DryBuffer, buffer, SamplesToDo, device->NumChannels);
                    buffer = (char*)buffer + SamplesToDo*device->NumChannels*sizeof(ALint);
                    break;
                case DevFmtUInt:
                    Write_ALuint((const ALfloatBUFFERSIZE*) device->DryBuffer, buffer, SamplesToDo, device->NumChannels);
                    buffer = (char*)buffer + SamplesToDo*device->NumChannels*sizeof(ALuint);
                    break;
                case DevFmtFloat:
                    Write_ALfloat((const ALfloatBUFFERSIZE*) device->DryBuffer, buffer, SamplesToDo, device->NumChannels);
