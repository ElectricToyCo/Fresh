...
#include "../../Gamepad.h"
#include "../../FreshCore/FreshEssentials.h"
#include "../../FreshCore/FreshRange.h"
#include "../../FreshCore/FreshMath.h"
...
#define REG_STRING_MAX 256
	...
	void povToXY( DWORD pov, float* outX, float* outY ) 
	{
		if( pov == JOY_POVCENTERED ) 
		{
			*outX = *outY = 0;
...
#define SET_AXIS_RANGE( axisLetter )	\
	m_axisRanges[ Axis::axisLetter ].min = static_cast< float >( caps.w##axisLetter##min );	\
	m_axisRanges[ Axis::axisLetter ].max = static_cast< float >( caps.w##axisLetter##max );

#define HANDLE_AXIS_CHANGE( axisLetter )	\
	if( currentState.dw##axisLetter##pos != m_lastState.dw##axisLetter##pos ) 	\
	{	\
		onAxisChanged( axisLetter, currentState.dw##axisLetter##pos );	\
	}
	...
	public:
		...
		GamepadPayload( Gamepad& gamepad, unsigned int id, const JOYINFOEX& state, const JOYCAPS& caps )
		:	m_id( id )
		,	m_gamepad( &gamepad )
		,	m_lastState( state )
		{
			const size_t nAxes = caps.wNumAxes;
			m_nHatButtons = ( caps.wCaps & JOYCAPS_HASPOV ) ? 4 : 0;
			m_nButtons = caps.wNumButtons;

			release_trace( "Gamepad " << id << " has " << nAxes << " axes and " << m_nButtons << " buttons, along with " << m_nHatButtons << " hat buttons." );
			
			m_axisRanges.resize( std::max( nAxes, size_t( Axis::NUM )));
			SET_AXIS_RANGE( X )
			SET_AXIS_RANGE( Y )
			SET_AXIS_RANGE( Z )
			SET_AXIS_RANGE( R )
			...
			{
				std::make_pair(  4, Gamepad::Button::A ),
				std::make_pair(  5, Gamepad::Button::B ),
				std::make_pair(  6, Gamepad::Button::X ),
				std::make_pair(  7, Gamepad::Button::Y ),
				std::make_pair(  3, Gamepad::Button::DPadRight ),
				std::make_pair(  1, Gamepad::Button::DPadDown ),
				std::make_pair(  2, Gamepad::Button::DPadLeft ),
				std::make_pair(  0, Gamepad::Button::DPadUp ),
				std::make_pair( 12, Gamepad::Button::LStick ),
				std::make_pair( 13, Gamepad::Button::RStick ),
				std::make_pair(  8, Gamepad::Button::LBumper ),
				std::make_pair(  9, Gamepad::Button::RBumper ),
				std::make_pair( 10, Gamepad::Button::Back ),
				std::make_pair( 11, Gamepad::Button::Start ),
			};
			...
			std::map< size_t, Gamepad::Axis > axisMap
			{
				...
				std::make_pair( 4, Gamepad::Axis::RX ),
				std::make_pair( 3, Gamepad::Axis::RY ),
				std::make_pair( 2, Gamepad::Axis::LTrigger ),
				std::make_pair( 2, Gamepad::Axis::RTrigger ),
			};
		...
		bool updateStates()
		{
			...
			currentState.dwSize = sizeof( currentState );
			currentState.dwFlags = JOY_RETURNALL;
			MMRESULT result = ::joyGetPosEx( m_id, &currentState );
			...
			{
				HANDLE_AXIS_CHANGE( X )
				HANDLE_AXIS_CHANGE( Y )
				HANDLE_AXIS_CHANGE( Z )
				HANDLE_AXIS_CHANGE( R )
				HANDLE_AXIS_CHANGE( U )
				HANDLE_AXIS_CHANGE( V )

				if( currentState.dwPOV != m_lastState.dwPOV ) 
				{
					float pov[ 2 ];
					povToXY( currentState.dwPOV, &pov[ 0 ], &pov[ 1 ] );

					m_gamepad->setButtonValue( 0, pov[ 1 ] < 0 );
					m_gamepad->setButtonValue( 1, pov[ 1 ] > 0 );
					m_gamepad->setButtonValue( 2, pov[ 0 ] < 0 );
					m_gamepad->setButtonValue( 3, pov[ 0 ] > 0 );
				}
				
				if( currentState.dwButtons != m_lastState.dwButtons ) 
				{
					const auto delta = m_lastState.dwButtons ^ currentState.dwButtons;

					for( size_t buttonIndex = 0; buttonIndex < m_nButtons; ++buttonIndex ) 
					{
						unsigned int buttonMask = 1 << buttonIndex;
						if( ( delta & buttonMask ) != 0 )
						{
							const bool down = ( currentState.dwButtons & buttonMask ) != 0;
							m_gamepad->setButtonValue( m_nHatButtons + buttonIndex, down );	// Skipping the hat buttons, which come first.
						}
					}
				}
	...
	protected:
		...
		void onAxisChanged( Axis axis, uint newValue )
		{
			ASSERT( m_gamepad );

			const auto value = clamp( proportion( float( newValue ), m_axisRanges[ axis ] ) * 2.0f - 1.0f, -1.0f, 1.0f );

			m_gamepad->setAxisValue( axis, value );		
		}
	...
	private:
		...
		JOYINFOEX m_lastState;
		std::vector< Range< float >> m_axisRanges;
		size_t m_nButtons = 0;
		size_t m_nHatButtons = 0;
	};
...
namespace fr
{
	...
	{
		const unsigned int numPadsSupported = ::joyGetNumDevs();

		for( unsigned int deviceIndex = 0; deviceIndex < numPadsSupported; ++deviceIndex ) 
		{
			JOYINFOEX joystickInfo;

			joystickInfo.dwSize = sizeof( joystickInfo );
			joystickInfo.dwFlags = JOY_RETURNALL;
			unsigned int joystickID = JOYSTICKID1 + deviceIndex;

			if( ::joyGetPosEx( joystickID, &joystickInfo ) == JOYERR_NOERROR )
			{
				JOYCAPS caps;

				if( ::joyGetDevCaps( joystickID, &caps, sizeof( JOYCAPS )) == JOYERR_NOERROR )
				{
					// Have we already registered a joystick with this ID?
					//
					if( m_gamepads.end() != std::find_if( m_gamepads.begin(), m_gamepads.end(), [&]( Gamepad::ptr gamepad ) {
							const auto& payload = payloadForGamepad( gamepad );
							return payload.hasID(  joystickID );
						} ))
					{
						continue;
					}
...
#ifdef DEBUG
					auto payload = new GamepadPayload( *gamepad, joystickID, joystickInfo, caps );
					ASSERT( gamepad->payload() == payload );
#endif
