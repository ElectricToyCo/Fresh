...
namespace fr
{
	...
	protected:
		...
		void generateBufferAndNotifySounds();
		void destroyBuffers();
	...
	void AudioCueImpl::generateBufferAndNotifySounds()
	{
#if !FRESH_NULL_AUDIO
		FRESH_AUDIO_SYNCHRONIZE
		ASSERT( dispatch::onMainQueue() );
		...
		FreshLoadBuffer( m_bufferIDs.front(),
						...
						static_cast< const ALvoid* >( m_fileData.data() ),
						static_cast< ALsizei >( m_nBytes ),
	...
	void AudioCueImpl::load( const path& filePath, AudioCue::ErrorHandler&& errorHandler )
	{
		...
		auto loadCompletionCallback = [weakThis]( std::vector< unsigned char >&& fileBytes, AudioCue::Format format, double sampleRate )
		{
			ASSERT( dispatch::onMainQueue() );
			...
			if( const auto strongThis = weakThis.lock() )
			{
				...
				{
					strongThis->m_nBytes = fileBytes.size();
					...
					strongThis->generateBufferAndNotifySounds();
...
//		dev_trace( "Loading audio file '" << path << "' asynchronously." );
...
#endif
		{
//			dev_trace( "Loading audio file '" << path << "' synchronously." );
			asyncLoad( path, std::move( loadCompletionCallback ), std::move( errorHandler ));
		}
#endif	// !FRESH_NULL_AUDIO
	}
...
{ 	dispatch::mainQueue().async( std::make_shared< fr::dispatch::Block >( [=, errorHandler = std::move( errorHandler ), message = createString( msg )]()	\
	...
	void asyncLoad( path filePath, CompletionHandler&& completionHandler, AudioCue::ErrorHandler&& errorHandler )
	{
		...
		if( fileData.empty() == false )
		{
			...
			ASSERT( fileData.size() == static_cast< size_t >( nBytes ) );
			...
			dispatch::mainQueue().async( std::make_shared< fr::dispatch::Block >( [=, completionHandler = std::move( completionHandler ), fileData = std::move( fileData )]() mutable
																				 {
																					 completionHandler( std::move( fileData ), properFormat, sampleRate );
																				 }, DISPATCH_CODE_LOCATION ));
		}
	}
	...
	void asyncLoad( path filePath, CompletionHandler&& completionHandler, AudioCue::ErrorHandler&& errorHandler )
	{
		...
		ASSERT( fileData.size() == nBytes );
		ASSERT( nBytes == nBytesLoaded );

		dispatch::mainQueue().async( std::make_shared< fr::dispatch::Block >( [=, completionHandler = std::move( completionHandler ), fileData = std::move( fileData )]() mutable
																			 {
																				 completionHandler( std::move( fileData ), format, sampleRate );
																			 }, DISPATCH_CODE_LOCATION ));
